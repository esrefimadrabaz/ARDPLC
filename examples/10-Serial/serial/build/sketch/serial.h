#line 1 "C:\\Users\\Oguz\\Desktop\\pl\\WindowsFormsApp1\\examples\\10-Serial\\serial\\serial.h"
...
void CMP(float first, float last, bool &MF, bool &MM, bool &ML)
{
  if(dugum)
  {
    if(first > last){MF = true; MM=false; ML=false;}
    else if (first == last) {MM = true; MF = false; ML=false;}
    else if ( first < last) {ML = true; MM=false; MF=false;}
  }
}
void ZCMP(float first, float middle, float last, bool &MF, bool &MM, bool &ML)
{
  ...
  {
    if(first > last){MF = true; MM=false; ML=false;}
    else if ((first <= last) && (last <= middle)) {MM = true; MF = false; ML=false;}
    else if ( last > middle) {ML = true; MM=false; MF=false;}
  }
}
